...
export default function WeddingGate({ onOpen }: WeddingGateProps) {
  ...
  const handleOpenGate = () => {
    ...
    // Trigger music immediately on user click
    onOpen()
    window.dispatchEvent(new CustomEvent('wedding:play-music'))

    // Play gate opening animation, then remove after transition
    setTimeout(() => {
      setIsGone(true)
    }, 1200)
  }
  ...
  return (
    <div className={`wedding-gate-overlay ${isOpening ? 'gate-opening' : ''}`}>
      ...
      {/* Center Royal Wedding Envelope Card */}
      <div className="gate-envelope-card">
        {/* Wax Seal Stamp */}
        <div className="gate-wax-seal" onClick={handleOpenGate}>
        ...
        <p className="gate-welcome-note">
          Trân trọng kính mời Quý Khách cùng bước vào không gian ngày vui của chúng mình.
        </p>
