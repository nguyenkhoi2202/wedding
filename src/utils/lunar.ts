/**
 * Thuật toán tính Âm Lịch Việt Nam chính xác múi giờ GMT+7 (Hồ Ngọc Đức)
 */
...
function getNewMoonDay(k: number, timeZone: number): number {
  ...
  if (T < -4) {
    ...
    deltat = -0.000078 + 0.000135 * T + 0.00025 * T2
  } else {
    deltat = -0.000078 + 0.000135 * T + 0.00025 * T2
  }
...
export function getVietnameseWeekday(dateStr: string): string {
  if (!dateStr) return ''
...
export function getLunarDateString(dateStr: string): string {
  if (!dateStr) return ''
  let dd = 0
  let mm = 0
  let yy = 0
  ...
  try {
    ...
    return `Nhằm ngày ${dayStr} tháng ${monthStr}${leapStr} năm ${yearName}`
  } catch {
    return ''
  }
}
